...
| **sqlite-graphrag** | `skills/sqlite-graphrag/` | 基于 sqlite-graphrag 的本地私有知识库构建与 AI 记忆管理 |
...
| 脚本 | 功能 |
|---|---|
| `init_knowledge_base.py` | 初始化 SQLite 知识库，创建图谱 Schema |
| `ingest_documents.py` | 将文档/笔记导入知识库，自动分块+向量化 |
| `query_knowledge_base.py` | 语义检索（向量+全文混合）和图谱多跳推理查询 |
| `export_knowledge_base.py` | 导入/导出知识库数据，支持备份迁移 |

**依赖：**
```bash
# 需先安装 sqlite-graphrag CLI 二进制（从 GitHub Releases 下载）
pip install openai          # 用于 embedding 和 LLM 推理
```
...
| sqlite-graphrag | Python 3.8+，需安装 sqlite-graphrag CLI 二进制 + openai |
